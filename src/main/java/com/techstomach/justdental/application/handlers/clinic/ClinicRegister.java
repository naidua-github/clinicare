...
public class ClinicRegister implements Handler<RoutingContext> {

    private Vertx vertx;
    private ConfigHandle configHandle;
    private static Logger log = LoggerFactory.getLogger(ClinicRegister.class);

    public ClinicRegister(Vertx vertx) {
        this.vertx = vertx;
    ...
    public void handle(RoutingContext routingContext) {
        ...
        String result = DbHandle.getInstance().insertClinic(dbRequest, "");
        if(result == null)
        {
            DbResponse dbResponse = new DbResponse();
            dbResponse.setBody(new Body().setClinic(new ClinicResp(dbRequest)));
        ...
        else
        {
            ...
            dbResponse.setBody(new Body()
                    .setClinic(new ClinicResp(dbRequest))
                    .setError(new Error("CLINIC REGISTRATION FAILED", result)));
