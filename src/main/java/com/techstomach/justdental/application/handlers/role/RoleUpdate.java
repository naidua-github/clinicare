...
public class RoleUpdate implements Handler<RoutingContext> {

    private Vertx vertx;
    private ConfigHandle configHandle;
    private static Logger log = LoggerFactory.getLogger(RoleUpdate.class);

    public RoleUpdate(Vertx vertx) {
        this.vertx = vertx;
    ...
    public void handle(RoutingContext routingContext) {
        ...
        String result = DbHandle.getInstance().updateRole(dbRequest, "");
        if(result == null)
        {
        ...
        else
        {
            ...
            dbResponse.setBody(new Body()
                    ...
                    .setError(new Error("ROLE UPDATE FAILED", result)));
